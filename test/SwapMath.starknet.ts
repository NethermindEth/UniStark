    ...
    describe('#computeSwapStep', () => {
        ...
        it('exact amount out that gets capped at price target in one for zero', async () => {
         ...
         it('exact amount in that is fully spent in one for zero', async () => {
           ...
           const amountInPlusFee = toBN(amountIn).add(toBN(feeAmount));
           expect(amountInPlusFee.toString(), 'entire amount is used').to.eq(amount.toString());
        ...
        it('exact amount out that is fully received in one for zero', async () => {
          ...
          expect(toBN(amountOut).toString()).to.eq(amount.mul(-1).toTwos(256).toString());
    
          const priceAfterWholeOutputAmount = await sqrtPriceMath.getNextSqrtPriceFromOutput_fedf2b5f(
            price.toString(),
            liquidity.toString(),
            toUint256(amount.mul(-1).toTwos(256).toString()),
            zeroForOne.toString()
          );
        ...
        it('handles intermediate insufficient liquidity in zero for one exact output case', async () => {
          ...
          const results = await swapMath.computeSwapStep_100d3f74(
            ...
            liquidity.toString(),
            toUint256(new BN(amountRemaining).toTwos(256)),
            feePips.toString()
          );
          const sqrtQ = results[0];
          const amountIn = results[1];
